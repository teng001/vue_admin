    ...
    <el-table-column label="操作" v-if="treeType === 'normal'" width="260">
      <template scope="scope">
        <button type="button" class="el-button el-button--default el-button--small">
          <router-link :to="{ path: requestUrl + 'router/detail/' + scope.row.id }" tag="span">
            编辑
          </router-link>
        </button>
...
import { fetchPermissionDelete } from '@/api/permission'
//  import Vue from 'vue'
export default {
  name: 'tree-grid',
  props: {
  ...
  methods: {
    ...
    handleDelete(id) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        console.log(id)
        fetchPermissionDelete(id).then(response => {
          this.$message({
            type: 'success',
            message: '删除成功!'
          })
