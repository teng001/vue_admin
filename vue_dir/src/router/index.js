...
export function jsonToRouter(data) {
  const router = []
  for (const i in data) {
    const obj = {}
    ...
    if (data[i].component === 'Layout') {
      ...
      obj.component = _import(data[i].component)
    }
    obj.name = data[i].name
    obj.icon = data[i].icon
    if (obj.meta !== 'undefine') {
      obj.meta = data[i].meta
    }
    obj.noDropdown = data[i].noDropdown
    if (data[i].children) {
      obj.children = jsonToRouter(data[i].children)
    }
    router.push(obj)
  }
  return router
